module DwcOccurrenceSpecificationHelpers
  ...
  def expect_row_tally(results, imported: 0, errored: 0, not_ready: 0, unsupported: 0, failed: 0)
    ...
    expect(tally.fetch('Failed', 0)).to eq(failed)
    expect(results.length).to eq(imported + errored + not_ready + unsupported + failed)
  end
